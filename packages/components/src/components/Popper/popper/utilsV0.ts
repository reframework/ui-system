...
const getArrowPlacement = (placement: InternalPlacementTuple) => {
  const [placementX, placementY] = placement;
 ...
 *
 */
const getOverflow = (params: GetOverflowParams): AxisOverflow => {
  const { popperPosition, offsetParentRect, popperRect, viewportRect } = params;
...
const getFlippedOffset = (offset = 0) => {
